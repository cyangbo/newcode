<?php
	...
	echo "<script type='text/javascript'>alert('没有文件或目录!!');location.href='online.php';</script>";

	}
	//print_r($info);
	@$act=$_REQUEST['act'];
	@$filename=$_REQUEST['filename'];
	$redirect="online.php?path={$path}";
	if($act=="createFile"){
		//创建文件
...
EFO;
			...
			alertMes("文件没有内容,亲编辑再查看!", $redirect);
		}
		...
		//echo $content;
		$str=<<<EOF
		<form action='online.php?act=doEdit' method='post'>
...
EOF;
		...
		//完成重命名
		$str=<<<EFO
		<form action="online.php?act=doRename" method="post">
...
echo $str;
		...
		//复制文件
		$str=<<<EOF
		<form action="online.php?act=doCopyFile" method="post">
			将文件复制到:
			<input type="text" name="dstname" placeholder="将文件复制到"/>
			<input type="hidden" name="path" value="{$path}"/>
			<input type="hidden" name="filename" value='{$filename}'/>
...
EOF;
	echo $str;
		...
		//复制文件夹
		$str=<<<EOF
	<form action="online.php?act=doCopyFolder" method="post"> 
...
echo $str;
	...
	elseif ($act=="doCopyFile"){
		...
		//剪切文件
				$str=<<<EOF
	<form action="online.php?act=doCutFile" method="post"> 
...
echo $str;
	...
	//重命名文件夹
	$str=<<<EOF
			<form action="online.php?act=doRenameFolder" method="post"> 
...
echo $str;
	...
	alertMes($mes,$redirect);
			...
			$str=<<<EOF
	<form action="online.php?act=doCutFolder" method="post"> 
...
<script type="text/javascript">
	...
	function delFile(filename,path){
		if(window.confirm("您确定要删除嘛?删除之后无法恢复哟!!!")){
				location.href="online.php?act=delFile&filename="+filename+"&path="+path;
		}
	}
	function delFolder(dirname,path){
		if(window.confirm("您确定要删除嘛?删除之后无法恢复哟!!!")){
			location.href="online.php?act=delFolder&dirname="+dirname+"&path="+path;
		}
	}
	...
	function goBack($back){
		location.href="online.php?path="+$back;
	}
</script>
</head>
...
<div id="top">
	<ul id="navi">
		<li><a href="online.php" title="主目录"><span style="margin-left: 8px; margin-top: 0px; top: 4px;" class="icon icon-small icon-square"><span class="icon-home"></span></span></a></li>
...
?>
	<tr>
		...
		?>
		<a href="online.php?act=showContent&path=<?php echo $path;?>&filename=<?php echo $p;?>" ><img class="small" src="images/show.png"  alt="" title="查看"/></a>|
		<?php }?>
		<a href="online.php?act=editContent&path=<?php echo $path;?>&filename=<?php echo $p;?>"><img class="small" src="images/edit.png"  alt="" title="修改"/></a>|
		<a href="online.php?act=renameFile&path=<?php echo $path;?>&filename=<?php echo $p;?>"><img class="small" src="images/rename.png"  alt="" title="重命名"/></a>|
		<a href="online.php?act=copyFile&path=<?php echo $path;?>&filename=<?php echo $p;?>"><img class="small" src="images/copy.png"  alt="" title="复制"/></a>|
		<a href="online.php?act=cutFile&path=<?php echo $path;?>&filename=<?php echo $p;?>"><img class="small" src="images/cut.png"  alt="" title="剪切"/></a>|
		<a href="#"  onclick="delFile('<?php echo $p;?>','<?php echo $path;?>')"><img class="small" src="images/delete.png"  alt="" title="删除"/></a>|
		<a href="online.php?act=downFile&path=<?php echo $path;?>&filename=<?php echo $p;?>"><img class="small"  src="images/download.png"  alt="" title="下载"/></a>
		</td>		
	</tr>
<?php 
$i++;
...
?>
	<tr>
		...
		<td><?php echo date("Y-m-d H:i:s",fileatime($p));?></td>
		<td>
		<a href="online.php?path=<?php echo $p;?>" ><img class="small" src="images/show.png"  alt="" title="查看"/></a>|
		<a href="online.php?act=renameFolder&path=<?php echo $path;?>&dirname=<?php echo $p;?>"><img class="small" src="images/rename.png"  alt="" title="重命名"/></a>|
		<a href="online.php?act=copyFolder&path=<?php echo $path;?>&dirname=<?php echo $p;?>"><img class="small" src="images/copy.png"  alt="" title="复制"/></a>|
		<a href="online.php?act=cutFolder&path=<?php echo $path;?>&dirname=<?php echo $p;?>"><img class="small" src="images/cut.png"  alt="" title="剪切"/></a>|
