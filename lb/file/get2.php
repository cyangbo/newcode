		...
		foreach ( $array_file as $key => $value ) {
			...
			if (strlen ( $str ) > 0) {
				...
				if (preg_match ( $mode, $string )) {
					...
					//如果匹配不到,就运行字符串添加
					if ((! preg_grep ( '/.com\/$/', $scc )) && (! preg_grep ( '/.cn\/$/', $scc )) && (! preg_grep ( '/.net\/$/', $scc )) && (! preg_grep ( '/.org\/$/', $scc )) && (! preg_grep ( '/.cc\/$/', $scc )) && (! preg_grep ( '/.com$/', $scc )) && (! preg_grep ( '/.cn$/', $scc )) && (! preg_grep ( '/.net$/', $scc )) && (! preg_grep ( '/.org$/', $scc )) && (! preg_grep ( '/.cc$/', $scc ))) {
						$scc = $scc [0];
						$outstring = $outstring . $scc . "\r\n";
					}
