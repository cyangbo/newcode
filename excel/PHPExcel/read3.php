    ...
    {  
        ...
        {  
            ...
            $content = $current_sheet->getCell($SerialNum)->getValue();//获取excel文件里当前坐标下（文本框）的内容 
             if($c == 'F' && $r>0){
             	$myurlc =$current_sheet->getCell($SerialNum)->getHyperlink();
             	$myurl[$c][$r]['content'] = $myurlc; 
             	print_r($myurl);
             }
...
/*        if($c == 'F'){
        	for($k = 1; $k <= $all_row; $k++){
        		...
        		$myseriaNum = $c.$k;
        		$content1 = $current_sheet->getCell($SerialNum)->getHyperlink();
        		if(is_object($content1))  
	            {  
	                $content1 = $content1->__toString();  
	            } 
        		
        		$myurl[$c][$r]['content'] = $content1;
